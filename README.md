...
**Повторение pandas-исследований**

Повторение pandas-исследований находится в файле `tasks.py`

## 2

**Выделение таблиц с гендерами, образованием, должностями и профессиями**

Таблица *genders* создается в `create_genders.py`.

Аналогично в `create_educations.py` создается таблица *educations* и заполняется значениями *"Высшее", "Незаконченное высшее", "Среднее", "Среднее профессиональное"*.

Для сокращения разнообразия должностей и профессий применена NLP-библиотека *nltk*. Всё работает так же, как и в прошлом задании.

Полученные таблицы сохраняются в базе данных и csv-формате (для удобства просмотра результатов анализа. Их можно посмотреть в `csv-files/jobTitles.csv` и `csv-files/qualifications.csv`).

Для удобства создания новой таблицы works с верно настроенными вторичными ключами отформатируем изначальную csv таблицу works (`init_works.csv`). Форматирование проходит в файле `works_modify.py`, создается csv таблица `mod_works.csv`, в которой значения столбцов *jobTitles* и *qualifications* соответствуют отобранным должностям и профессиям соответственно.

В файле `create_works.py` создается таблица базы данных *works* с настроенными вторичными ключами и значениями из csv таблицы `mod_works.csv`.

## 3

**Выделение таблиц с навыками и доп. информацией**

Для отчистки столбцов данных от html-разметки написана функция на основе регулярного выражения:
...
Таблица с навыками формируется при помощи той же библиотеки *nltk* (файл `create_info.py`), только для нее также задействуются возможности этой библиотеки по анализу принадлежности слов ( метод `nltk.pos_tag([word], lang='rus')`). Отбираем в финальную таблицу прилагательные и английские слова, ручным способ исключая слова-ошибки.

- Полученная таблица добавлена в базу данных и сохранена в формате csv (`csv-files/skills.csv`);
- В `works_modify.py` `mod_works.csv` скорректирована под полученный лист навыков;
- На основе `mod_works.csv` скорректирована таблица *works* базы данных.

Для анализа таблицы с доп. информацией возможностей библиотеки *nltk* не хватает, так как она очень грубо оценивает к какой части речи относится слово.

Поэтому была использована утилита разработки Яндекса - `Mystem 3`, и пакет для подключения к ней из кода Python - [pymystem3](https://github.com/nlpub/pymystem3).

При помощи `Mystem` были отобраны прилагательные из столбца *otherInfo*, список сохранен в базу данных и `csv-files/otherInfo.csv`. По аналогии со всеми прошлыми пунктами обновлена `mod_works.csv` (`works_modify.py`) и таблица *works* в базе данных (`create_works.py`).
